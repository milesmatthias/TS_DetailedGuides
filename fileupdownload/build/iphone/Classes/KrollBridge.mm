...
@implementation fileupdownloadObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_fileupdownload);
	RELEASE_TO_NIL(modules);
...
-(void)injectPatches
{
	// called to inject any fileupdownload patches in JS before a context is loaded... nice for 
...
{
	// create fileupdownload global object
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_fileupdownload = [[fileupdownloadObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_fileupdownload];
	
	NSString *_fileupdownloadNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _fileupdownloadNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_fileupdownload valueForKey:name];
...
-(id)require:(KrollContext*)kroll path:(NSString*)path
{
	...
	@throw [NSException exceptionWithName:@"org.fileupdownload.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
