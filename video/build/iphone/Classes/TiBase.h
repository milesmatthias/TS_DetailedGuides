...
// in simulator we redefine to format for video Developer console
...
{\
	NSString * exceptionName = [@"org.video." stringByAppendingString:NSStringFromClass([self class])];\
...
+ (void) throwException:(NSString *) reason subreason:(NSString*)subreason location:(NSString *)location\
{\
	NSString * exceptionName = @"org.video";\
	NSString * message = [NSString stringWithFormat:@"%@. %@ %@",reason,(subreason!=nil?subreason:@""),(location!=nil?location:@"")];\
	NSLog(@"[ERROR] %@",message);\
	if ([NSThread isMainThread]==NO) {\
		@throw [NSException exceptionWithName:exceptionName reason:message userInfo:nil];\
...
enum {
	UIvideoNativeItemNone = -1, 
	UIvideoNativeItemSpinner = -2,
	UIvideoNativeItemProgressBar = -3,
	
	UIvideoNativeItemSlider = -4,
	UIvideoNativeItemSwitch = -5,
	UIvideoNativeItemMultiButton = -6,
	UIvideoNativeItemSegmented = -7,
	
	UIvideoNativeItemTextView = -8,
	UIvideoNativeItemTextField = -9,
	UIvideoNativeItemSearchBar = -10,
	
	UIvideoNativeItemPicker = -11,
	UIvideoNativeItemDatePicker = -12,
	
	UIvideoNativeItemInfoLight = -13,
	UIvideoNativeItemInfoDark = -14,
	
	UIvideoNativeItemDisclosure = -15,
	
	UIvideoNativeItemContactAdd = -16
};
